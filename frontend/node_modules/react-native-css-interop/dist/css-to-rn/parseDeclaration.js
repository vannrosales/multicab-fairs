...
exports.parseUnresolvedColor = parseUnresolvedColor;
const shared_1 = require("../shared");
const normalize_selectors_1 = require("./normalize-selectors");
const unparsedPropertyMapping = {
...
function parseDeclaration(declaration, options) {
    const { addStyleProp, addTransformProp, handleStyleShorthand, handleTransformShorthand, addAnimationProp, addContainerProp, addTransitionProp, addWarning, } = options;
    ...
    if (declaration.property === "unparsed") {
        ...
        let property = unparsedPropertyMapping[declaration.value.propertyId.property] ||
        ...
        if (unparsedRuntimeFn.has(property)) {
            ...
            if (!(0, shared_1.isDescriptorArray)(args)) {
                args = [args];
            }
            return addStyleProp(property, [{}, `@${(0, normalize_selectors_1.toRNProperty)(property)}`, args]);
        }
        return addStyleProp(property, parseUnparsed(declaration.value.value, parseOptions));
    }
    ...
    switch (declaration.property) {
        ...
        case "transform": {
            ...
            for (const transform of declaration.value) {
                switch (transform.type) {
                    case "perspective":
                        addTransformProp("perspective", parseLength(transform.value, parseOptions));
                        break;
                    case "translate":
                        addTransformProp("translateX", parseLengthOrCoercePercentageToRuntime(transform.value[0], "rnw", parseOptions));
                        addTransformProp("translateY", parseLengthOrCoercePercentageToRuntime(transform.value[1], "rnh", parseOptions));
                        break;
                    case "translateX":
                        addTransformProp("translateX", parseLengthOrCoercePercentageToRuntime(transform.value, "rnw", parseOptions));
                        break;
                    case "translateY":
                        addTransformProp("translateY", parseLengthOrCoercePercentageToRuntime(transform.value, "rnh", parseOptions));
                        break;
                    case "rotate":
                        addTransformProp("rotate", parseAngle(transform.value, parseOptions));
                        break;
                    case "rotateX":
                        addTransformProp("rotateX", parseAngle(transform.value, parseOptions));
                        break;
                    case "rotateY":
                        addTransformProp("rotateY", parseAngle(transform.value, parseOptions));
                        break;
                    case "rotateZ":
                        addTransformProp("rotateZ", parseAngle(transform.value, parseOptions));
                        break;
                    case "scale":
                        ...
                        break;
                    case "scaleX":
                        addTransformProp("scaleX", parseLength(transform.value, parseOptions));
                        break;
                    case "scaleY":
                        addTransformProp("scaleY", parseLength(transform.value, parseOptions));
                        break;
                    case "skew":
                        addTransformProp("skewX", parseAngle(transform.value[0], parseOptions));
                        addTransformProp("skewY", parseAngle(transform.value[1], parseOptions));
                        break;
                    case "skewX":
                        addTransformProp("skewX", parseAngle(transform.value, parseOptions));
                        break;
                    case "skewY":
                        addTransformProp("skewY", parseAngle(transform.value, parseOptions));
                        break;
                    case "translateZ":
                    case "translate3d":
                    case "scaleZ":
        ...
        case "translate":
            addStyleProp("translateX", parseTranslate(declaration.value, "x", parseOptions));
            addStyleProp("translateX", parseTranslate(declaration.value, "y", parseOptions));
            return;
        case "rotate":
...
function reduceParseUnparsed(tokenOrValues, options) {
...
function parseUnparsed(tokenOrValue, options) {
    ...
    switch (tokenOrValue.type) {
        ...
        case "var": {
            const args = [tokenOrValue.value.name.ident];
        ...
        case "function": {
            switch (tokenOrValue.value.name) {
                ...
                case "min":
                case "clamp":
                    return parseCalcFn(tokenOrValue.value.name, tokenOrValue.value.arguments, options);
                default: {
...
function parseColor(color, options) {
    ...
    switch (color.type) {
        case "rgb": {
            ...
            if (color.alpha !== 1) {
                hexValues.push(Math.round(color.alpha * 255)
                    .toString(16)
                    .padStart(2, "0"));
            }
...
function parseLengthOrCoercePercentageToRuntime(value, runtimeName, options) {
    if (options.features.transformPercentagePolyfill &&
...
function parseEnv(value, options) {
...
function parseCalcFn(name, tokens, options) {
    const args = parseCalcArguments(tokens, options);
    if (args) {
        return [{}, name, args];
    }
}
function parseCalcArguments([...args], options) {
    const parsed = [];
    let mode;
...
function parseTranslate(translate, prop, options) {
    if (translate === "none") {
        return 0;
    }
    return parseLength(translate[prop], options);
}
function parseScale(translate, prop, options) {
    if (translate === "none") {
        return 0;
    }
    return parseLength(translate[prop], options);
}
function parseUnresolvedColor(color, options) {
    switch (color.type) {
        case "rgb":
            return [
                {},
